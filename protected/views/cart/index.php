...
<center>
	<h1 style="font-size:12px; margin: 5px 0 0 5px;">
		...
		<a style="text-decoration: none;color:green;" href="<?php echo $this->createUrl("site/");?>">Silahkan belanja</a>
	</h1>	
</center>
<?php }else{ ?>
...
<?php $i=1;foreach($data as $model): $sum=$sum+($model['harga'] * $model['qty'])?>
	<?php echo CHtml::hiddenField('id' . $i, $model['id'], array('maxlength' => 3, 'style' => "width:20px;text-align:center")); ?>
		...
		<td><b><?php echo $model['nama_produk']; ?></b><br>
			<?php echo CHtml::image(Yii::app() -> request -> baseUrl . '/images/produk/thumbs/' . $model['gambar'] . '', '', array("style" => "width:82px;", )); ?>
			</td>
...
<?php $i++;
	...
	<tr id="clean">
		<td></td>
		<td><?php echo CHtml::button('Belanja lagi', array('submit' => array('site/'))); ?></td>
		<td>&nbsp;</td>
