		...
		if(isset($_GET['confirm'])){
			...
			if(isset($_POST['Confirmpayment'])){
				...
				if($model->save()){
					...
					$modelOrder->payment_status = 0;
					/*simpan perubahan*/
					$modelOrder->save();
					/*setFlash untuk informasi sukses konfirmasi pesanan*/
					Yii::app()->user->setFlash('success', "Pesanan anda dengan nomor pemesanan #".$order_code." telah berhasil dikonfirmasi!"); 
...
/*public function actionMasukkan {
$update_barang = Product::model()->findAllByPk();
            foreach ($update_barang as $b) {
                $hasil_kurang = $b->stock - $qty;
                $b->stok = $hasil_kurang;
                $b->save();
            }	*/
